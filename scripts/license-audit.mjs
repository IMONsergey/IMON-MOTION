...
const parseQuotedValue = (raw) => {
  const value = raw.trim();
  if (value.startsWith('"') && value.endsWith('"')) return value.slice(1, -1).replace(/\\"/g, '"');
  return value;
};

// SOURCES.lock.yaml is generated by us and intentionally simple. Parse the known
// two-level structure line-by-line instead of relying on a fragile multiline regex.
const parseSourcesLock = (text) => {
  const sources = {};
  let inSources = false;
  let current = null;
  ...
  for (const line of text.split(/\r?\n/)) {
    ...
    const donorMatch = line.match(/^  ([A-Za-z0-9_.-]+):\s*$/);
    if (donorMatch) {
      current = donorMatch[1];
      sources[current] = {};
      continue;
    }
    ...
    sources[current][property[1]] = parseQuotedValue(property[2]);
  }

  return sources;
};

const lockSources = parseSourcesLock(lock);
const indexedDonors = Object.keys(catalog.donors ?? {}).sort();
const parsedDonors = Object.keys(lockSources).sort();
const missingFromLock = indexedDonors.filter((donor) => !lockSources[donor]);
const extraInLock = parsedDonors.filter((donor) => !catalog.donors?.[donor]);
if (missingFromLock.length || extraInLock.length) {
  throw new Error(
    `SOURCES.lock/index donor mismatch. Missing in lock: ${missingFromLock.join(', ') || 'none'}; extra in lock: ${extraInLock.join(', ') || 'none'}`,
  );
}
...
const assetExtensions = new Set([
  '.mp3', '.wav', '.m4a', '.aac', '.ogg', '.mp4', '.mov', '.webm', '.png', '.jpg', '.jpeg', '.webp', '.gif', '.avif',
  '.glb', '.gltf', '.pdf', '.woff', '.woff2', '.ttf', '.otf',
]);

const sources = {};
for (const donor of indexedDonors) {
  const meta = lockSources[donor] ?? {};
  const repository = meta.repository ?? '';
  const snapshotPath = meta.path ?? `upstream/${donor}`;
  const licenseField = meta.license_files ?? '';
  const licenseFiles = licenseField.split(',').map((item) => item.trim()).filter(Boolean);

  const detected = [];
  for (const licenseFile of licenseFiles) {
    const full = path.resolve(root, snapshotPath, licenseFile);
    const allowedRoot = path.resolve(root, snapshotPath) + path.sep;
    if (!full.startsWith(allowedRoot)) {
      detected.push({ file: licenseFile, exists: false, classification: 'invalid-path', error: 'path escapes donor snapshot' });
      continue;
    }
    ...
    try {
      ...
      if (!stat.isFile()) {
        detected.push({ file: licenseFile, exists: false, classification: 'not-a-file' });
        continue;
      }
  ...
  try {
    ...
    for (const file of donorIndex.files ?? []) {
      ...
      if (assetSamples.length < 12) assetSamples.push(file.path);
    }
  } catch (error) {
    throw new Error(`Failed to read donor index for ${donor}: ${error instanceof Error ? error.message : String(error)}`);
  }

  const hasLicenseFile = detected.some((entry) => entry.exists);
  const recognized = detected
    .filter((entry) => entry.exists && entry.classification !== 'unclassified')
...
const report = {
  ...
  summary: {
    ...
    missingLicenseFile: missingLicense,
    unclassifiedLicense,
...
console.log(
  `License audit OK: ${Object.keys(sources).length} donors parsed; ${report.summary.withDetectedLicenseFile} with detected license file; ` +
  `${missingLicense.length + unclassifiedLicense.length} review-required license state(s); ${assetHeavy.length} donor(s) with bundled assets.`,
);
